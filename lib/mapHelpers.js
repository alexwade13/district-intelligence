...
export const updateTractColors = (
  map,
  shapes,
  data,
  selectedIndicator,
  ...
  const { demographic: demographicColorScales } = colorScales
  const tractData = data['assembly-tract-data']

  if (!tractData || !shapes['census-tracts']) return

  Object.keys(shapes['census-tracts']).forEach((tractId) => {
    let color = '#cccccc'
    let opacity = 0.3

    let tractDemographics = null
    for (const adId in tractData) {
      if (tractData[adId].tract_demographics && tractData[adId].tract_demographics[tractId]) {
        tractDemographics = tractData[adId].tract_demographics[tractId]
        break
      }
    }

    if (tractDemographics) {
      const value = tractDemographics[selectedIndicator]
    ...
    map.current.setFeatureState(
      {
        ...
        id: shapes['census-tracts'][tractId].id,
      },
      {
        color,
        opacity,
        'line-width': selectedTract === tractId ? 1.5 : 0.1,
      },
    )
  })
}
...
export const updateDistrictColors = (
  ...
  if (scale === 'Assembly district') {
    ...
    Object.keys(shapes['assembly-districts']).forEach((k) => {
      ...
      const isSelectedForDrilldown = selected[scaleLookup[scale]] === k
      ...
      map.current.setFeatureState(
        ...
        {
          color,
          opacity: isSelectedForDrilldown ? 0 : opacity,
          'line-width': isSelectedForDrilldown ? 1.5 : 0.5,
        },
      )
    })
  }
...
export const updateDrilledDownTracts = (
  map,
  shapes,
  data,
  selectedIndicator,
  ...
  if (!tractData || !selectedAD || !tractData[selectedAD]) return
  
  const adTracts = tractData[selectedAD].tract_demographics || {}
  
  Object.keys(shapes['census-tracts']).forEach((tractId) => {
    if (adTracts[tractId]) {
      const demographics = adTracts[tractId]
      const value = demographics[selectedIndicator]
